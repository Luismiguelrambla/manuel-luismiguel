...
async function main() {
  try {
    ...
    await connection.query(`
            ...
            -- Manu deja comentado para revisión
    ...
    console.log("tabla equipamiento");
    ...
    for (let index = 0; index < usersAdmin; index++) {
      ...
      await connection.query(`
                ...
                VALUES("${formatDateToDB(
                  now
                )}", "Manuel", "Cordero", "corderovivero.manuelalejandro@gmail.com", SHA2("${
    ...
    for (let index = 0; index < espacios; index++) {
      ...
      await connection.query(`
                ...
                VALUES("${formatDateToDB(now)}", "${name}", "${random(
        1,
        5
      )}", "${descripcion}", "${aforo}", "${random(1, hotel)}")
            `);
    }
    ...
    for (let index = 0; index < equipamiento; index++) {
      await connection.query(`
                ...
                VALUES("${random(0, 1)}", "${random(0, 1)}", "${random(
        0,
        1
      )}", "${random(0, 1)}", "${random(0, 1)}", "${random(0, 1)}", "${random(
        0,
        1
      )}", "${random(0, 1)}", "1")
            `);
    ...
    for (let index = 0; index < reservas; index++) {
      ...
      await connection.query(`
                ...
                VALUES("${random(1, users)}", "${random(
        1,
        espacios
      )}", "${formatDateToDB(now)}", "${formatDateToDB(
        fechaLlegada
