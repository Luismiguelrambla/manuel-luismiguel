...
const deleteEspacio = async (req, res, next) => {
  ...
  try {
    ...
    const [photos] = await connection.query(`SELECT fotos FROM fotosEspacios WHERE idEspacios = ?`, [id]);

    // borrar las posibles fotos de la tabla fotosEspacios
    await connection.query(`DELETE FROM fotosEspacios WHERE idEspacios = ?`, [id]);
