    ...
    try {
        ...
        for (let index = 0; index < users; index++) {
            ...
            await connection.query(`
                ...
                now
            )}", "${name}", "${surname}", "${email}", SHA2("${password}", 512), "${phone}", "${random(1, 4)}", "${formatDateToDB(fecha)}", true)
            `);
        }
        ...
        for (let index = 0; index < usersAdmin; index++) {
            ...
            await connection.query(`
                INSERT INTO usuarios(admin, fechaRegistro, nombre, apellidos, correo, contraseña, telefono, metodoDePago, fechaNacimiento, active)
                VALUES("1", "${formatDateToDB(now)}", "${name}", "${surname}", "${email}", SHA2("${password}", 512), "${phone}", "${random(1, 4)}", "${formatDateToDB(fecha)}", true)
            `);
        }
        ...
        for (let index = 0; index < espacios; index++) {
            ...
            await connection.query(`
                ...
                VALUES("${formatDateToDB(now)}", "${name}", "${random(1, 5)}", "${descripcion}", "${aforo}", "${random(1, hotel)}")
            `);
        }
        ...
        for (let index = 0; index < equipamiento; index++) {
            ...
            await connection.query(`
                ...
                VALUES("${random(0, 1)}", "${random(0, 1)}", "${random(0, 1)}", "${random(0, 1)}", "${random(0, 1)}", "${random(0, 1)}", "${random(0, 1)}", "${random(0, 1)}", "1")
            `);
        }
        ...
        for (let index = 0; index < reservas; index++) {
            ...
            await connection.query(`
                ...
                VALUES("${random(1, users)}", "${random(1, espacios)}", "${formatDateToDB(now)}", "${formatDateToDB(fechaLlegada)}", "${formatDateToDB(fechaSalida)}")
            `);
        }
